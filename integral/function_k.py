...
Specifically:
- Sc(c) returns the list of all (up to) 2^18 sums of distinct powers of 3 and of 4, 
  the latter being multiplied by c. This is used to compute the gaps in the set A_c 
  end finally the value of the function k(c) in the two implementations below.
- k_full(c) runs over all pairs (a,b) of adjacent values in S(c), and checks for holes 
  <=> a + d < b; if such a hole is detected, the size of the gap is added to a total 
  "offset", which gives the value of b - Ac(b), where Ac(b)=measure([0,b] &inter; Ac).
  The function returns a tuple with
      k(c) = min { (x - offset at x)/x ; x ∈ Sc(c) }
           = 1 - max { (offset at x)/x ; x ∈ Sc(c) }
  and some other information, such as the x-value where this min. is reached, etc.
- k_min is a more efficient function to compute just k(c), for numerical integration 
  and plotting, for example.
"""
supercycles = 1  # use supercycles by default
...
# simplified & optimized function to compute just k_min
def k_min(c):
    "Return k(c) = inf { measure(A_c \cap [0,x]) / x ; 0 < x < max A_c }."
    # Use conservative bounds for 'limit' (see A_c): to ensure that 
    # the higher limit is used for c-values at transition boundary,
    # the c-values used in the if-else clauses below must be rounded
    # towards the region where only the lower limit is required.
    '''#  more explicit, but needs more comparisions:
    ...
    for b in Sc(c, limit = limit, append_3powR = False):
        if b > a+d:
            o += b - a - d  # add the gap to the offset
            if o/b > t:
                t = o/b
            if b >= limit: break
        a = b
...
# This list will be used to make good choices of where to split up
# the numerical integration in separate integrals. Therefore, we
# may use in some cases a value slightly to the left or to the right
# of the exact location of an extemum.
# Also, we have commented out some of the "points of interest"
# if it appears not useful to split up the integral there.

if 'sqrt' not in vars(): from math import sqrt
points_of_interest = {
        ...
        'max_ll':   1.20135498, # k=0.76767537    # upper bound of LL, but rather a min of k !
        ...
        'min3_end': 1.2695,     # k=0.813 , slope becomes flat again
        'end':      4/3, #=1.333333, # k=1183/1458 = 0.8113854595
}
